    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                Console.WriteLine("接收到客户端 " + sc.RemoteEndPoint.ToString() + " 的连接！");
                //握手
                int length = sc.Receive(buffer); //接收客户端的握手信息
                sc.Send(PackHandShakeData(GetSecKeyAccetp(buffer, length)));
                Console.WriteLine("已发送握手协议...");
                //接收客户端数据
        ...
        private static string GetSecKeyAccetp(byte[] handShakeBytes, int bytesLength)
        {
            ...
            foreach (var item in handShakeText.Split('\n'))
            {
                if (item.Contains("Sec-WebSocket-Key"))
                {
                    key = item.Split(':')[1].Trim();
                    break;
                }
            }
